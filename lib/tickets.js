...
module.exports = function tickets(opts, callback) {
  ...
  if (cfg.jira.length > 1) {
    ...
    var jcfg = _.assign(cfg, {
      url: cfg.jira[0].url,
      keys: cfg.jira[0].keys || cfg.keys,
      username: cfg.jira[0].username,
      password: cfg.jira[0].password
    });

    delete jcfg.jira;

    lookupJira(jcfg, callback);
  }
      ...
      request({
        ...
        if (err) {
          issue.error = util.format(
            'Request to Jira for issue "%s" failed with: %s.',
            issue.key, err
          );
        ...
        if (!callback) {
          ...
          if (issue.error) {
            ...
            console.log(chalk.cyan(U.strPadRight('URL', 15)) + cfg.url + '/browse/' + issue.key);
